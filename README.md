...
- React Router v7
- Express backend (local dev) that calls the Anthropic API to triage grievances, mirrored as
...
The frontend and the AI backend (`api/health.ts`, `api/prioritize.ts`, `api/prioritize-batch.ts`)
deploy together as a single Vercel project — no separate backend hosting needed.
...
3. Before deploying, add an environment variable: **Settings → Environment Variables** →
   `ANTHROPIC_API_KEY` = your key. (Optionally also set `ANTHROPIC_MODEL`, default is
   `claude-haiku-4-5-20251001`.) Without this key, AI prioritization falls back to the
   rule-based logic — the app still works, just without real AI triage.
4. Click **Deploy**. You'll get a `https://<project>.vercel.app` URL with the frontend and the
