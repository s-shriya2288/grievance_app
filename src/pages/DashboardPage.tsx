  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-8">
      <motion.div variants={item} className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Hi {employee?.name.split(' ')[0]}, welcome back</h1>
          <p className="mt-1 text-sm text-slate-500">Here's what's happening with your grievances.</p>
        </div>
        <Link
          to="/grievances/new"
      ...
      <motion.div variants={item} className="rounded-xl border border-slate-200 bg-white">
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          <h2 className="font-semibold text-slate-900">Recent Grievances</h2>
          <Link to="/grievances" className="text-sm font-medium text-brand-600 hover:text-brand-700">
            View all
          </Link>
        </div>
        <ul className="divide-y divide-slate-100">
            ...
            <li key={grievance.id}>
              ...
              >
                <div className="min-w-0">
                  <p className="truncate font-medium text-slate-800">{grievance.subject}</p>
          ...
          {grievances.length === 0 && (
            <li className="px-5 py-10 text-center text-sm text-slate-400">No grievances submitted yet.</li>
          )}
        </ul>
      </motion.div>
    </motion.div>
