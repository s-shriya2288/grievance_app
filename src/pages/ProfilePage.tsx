...
function ChangeEmailForm() {
  ...
  const handleSendCode = async (event: FormEvent) => {
    ...
    try {
      ...
      setStage('code-sent')
      setMessage({ type: 'success', text: `We sent a 6-digit code to ${newEmail.trim()}.` })
    } catch (err) {
      setMessage({ type: 'error', text: err instanceof ApiError ? err.message : 'Could not start the email change.' })
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleConfirm = async (event: FormEvent) => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: err instanceof ApiError ? err.message : 'Could not confirm the code.' })
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <form
      onSubmit={stage === 'idle' ? handleSendCode : handleConfirm}
      className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900"
    >
      <h2 className="mb-4 font-semibold text-slate-900 dark:text-slate-100">Change Email</h2>
      <p className="mb-4 text-sm text-slate-500 dark:text-slate-400">
        Current email: <span className="font-medium text-slate-700 dark:text-slate-300">{user?.email}</span>
      </p>

      {stage === 'idle' ? (
        <div>
          <label className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">New company email</label>
          <input
            type="email"
            required
            value={newEmail}
            onChange={(e) => setNewEmail(e.target.value)}
            placeholder="you@dalmiacement.com"
        ...
        <div>
          {devOtp && (
            <p className="mb-3 rounded-lg border border-amber-200 bg-amber-50 px-3.5 py-2.5 text-xs text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          type="submit"
          disabled={saving}
          className="rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-brand-700 disabled:cursor-not-allowed disabled:opacity-60"
        >
          {stage === 'idle' ? (saving ? 'Sending…' : 'Send Code') : saving ? 'Confirming…' : 'Confirm New Email'}
        </button>
        {stage === 'code-sent' && (
          <button
            type="button"
